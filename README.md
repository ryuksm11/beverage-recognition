...
Trained on EfficientNet-B0 with two-phase fine-tuning:

| Metric | Result |
|--------|--------|
| Test accuracy | 82.98% |
| Per-class F1 (min) | 0.762 (Tropicana) |
| Per-class F1 (max) | 0.889 (Red Bull) |

All 9 classes met the ≥ 0.65 F1 gate.

> **Note:** Model is trained on scraped web product images (clean, studio backgrounds).
> Real-world photos taken at angles or with busy backgrounds may score lower confidence.
...
Saves `models/best_checkpoint.pth`, `models/confusion_matrix.png`, `models/training_curves.png`.
